...
export const ChatRoomComponent = ({ chatId, onBack }) => {
    ...
    useEffect(() => {
        ...
        // Handle saved messages separately
    ...
    useEffect(() => {
        ...
        const db = getDatabase(Firebase);
        const messagesRef = chatId === "saved-messages"
            ? ref(db, `/userMessages/saved-${userData.uid}`)
            : ref(db, `/userMessages/${chatId}`);
    ...
    const handleSendMessage = () => {
        ...
        let messagesPath, receiverId;
        if (chatId === "saved-messages") {
            messagesPath = `/userMessages/saved-${userData.uid}`;
            receiverId = userData.uid;
        } else {
            messagesPath = `/userMessages/${chatId}`;
        ...
        if (chatId !== "saved-messages") {
            ...
            set(ref(db, `/userChatList/${receiverId}/${chatId}/lastMessage`), messageText);
            set(ref(db, `/userChatList/${receiverId}/${chatId}/lastTimestamp`), createdAt);
        }
